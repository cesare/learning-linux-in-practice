...
static void show_usage(char** argv) {
  fprintf(stderr, "Usage: %s [options] <total[ms]> <resolution[ms]>\n", argv[0]);
...
static Config* parse_arguments(int argc, char** argv) {
  ...
  int64_t total_parsed = strtol(argv[offset + 0], NULL, 10);
  int64_t resol_parsed = strtol(argv[offset + 1], NULL, 10);
